...
import { closeIssue, deleteProject } from '../core/github-project.js';
...
export async function resetCommand(options: { force?: boolean; team?: string }): Promise<void> {
  ...
  if (teamNames.length === 0) {
    logger.error('No sync state found. Nothing to reset.');
    process.exit(1);
  }
  ...
  for (const teamName of teamNames) {
    ...
    if (!options.force) {
      const ok = await confirm(`Close all issues, DELETE the GitHub Project, and remove sync state for team "${teamName}"?`);
      if (!ok) {
        logger.info('Skipped.');
        continue;
      }
    ...
    logger.success(`Team "${teamName}" reset complete.`);
  }
}
